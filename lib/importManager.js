...
class ImportManager {
    ...
    async handleImportMessage(msg) {
        try {
            this.adapter.log.info(`Import message received: ${JSON.stringify(msg.message)}`);
            let messageData;

            // Check for custom delimiter format (to handle CSV newlines safely)
            if (typeof msg.message === 'string' && msg.message.includes('###DATA###')) {
                const parts = msg.message.split('###DATA###');
                messageData = JSON.parse(parts[0]);
                messageData.content = parts[1];
            } else {
                messageData = typeof msg.message === 'string' ? JSON.parse(msg.message) : msg.message;
            }

            let { utility, type, content, customName, unit } = messageData;

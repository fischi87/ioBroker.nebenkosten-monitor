...
class MultiMeterManager {
    ...
    async initializeMeter(type, meterName, config, displayName) {
        ...
        const nowIso = calculator.formatDateString();
    ...
    async updateCosts(type, meterName, config) {
        ...
        if (yearStartState && yearStartState.val) {
            ...
            if (yearStartDate && !isNaN(yearStartDate.getTime())) {
                const now = new Date();
                const daysSinceYearStart = Math.floor((now - yearStartDate) / (1000 * 60 * 60 * 24));
        ...
        const totalYearlyCost = yearlyCost + (config.grundgebuehr || 0) * 12 + annualFeeAccumulated;
        await this.adapter.setStateAsync(
